...
持续负收益不自动意味着反向交易有正收益。管线先后执行三类互不覆盖的研究：

1. **单一岗位考试**：每位教练只用开发集，从 `标的 × 周期 × 最大成本/R × 最低 RR × 行情方向` 中选一个岗位；验证集只负责第一次拒绝，最终留出只负责最后审计，不能回头换岗。开发至少 100 笔，验证与最终留出各至少 40 笔且每笔 EV 不低于 `+0.03R`。
2. **简单反向实验**：保留原镜头作为方向触发器，但方向取反，并按新方向重新生成结构进场、止损和目标。它不是把原策略收益乘以 `-1`。
3. **成本感知补救**：保留原方向，从 `最大成本/R = 0.12、0.18、0.25、0.35`、`最低 RR = 1.2、1.5、2.0`、`全部/避开逆势/只做顺势` 组合出 36 套少交易方案。
...
| 研究路线 | 通过开发+验证 | 最终结论 |
|---|---:|---|
| 单一岗位考试 | 3 / 14 | Brooks、缠师、锚定 VWAP 通过验证；最终留出无人达到 `+0.03R`，不开放独立下单权；另 3 位缺必要输入未参赛 |
...
团队票位同样不能看最终留出。训练预先声明了全体镜头、仅价格结构与纪律、2 个高重要度票位、5 个高重要度票位四个候选，只用验证集的覆盖和目标函数选择。“5 个票位”最终对应动量、均值回归、锚定 VWAP、道氏四位计划席加总共识，并压制其他教练票位；其他教练仍可在界面解释结构，但不能影响门控分数。

| 验证阶段候选 | 验证样本 | 验证范围加权 EV | 选择结果 |
|---|---:|---:|---|
| 全体方法镜头 | 213 | +0.032R | 未选 |
| 仅价格结构与执行纪律 | 90 | -0.102R | 覆盖不足且为负 |
| 2 位计划席 | 197 | +0.011R | 未达到 200 笔覆盖门槛 |
| 4 位计划席 + 总共识 | 200 | +0.044R | 验证目标最高，冻结后进入最终留出 |

| 阶段 | 通过门控的计划 | 胜率 | 每笔 EV | 范围加权 EV | Profit Factor | 最大回撤 | 累计 R |
|---|---:|---:|---:|---:|---:|---:|---:|
| 验证 | 200 | 58.5% | +0.075R | +0.044R | 1.14 | -15.26R | +15.04R |
| 最终留出 | 238 | 56.3% | +0.053R | +0.054R | 1.10 | -13.68R | +12.51R |

验证 AUC 为 0.6045，最终留出 AUC 为 0.5933，属于有限而不是强区分能力。全体方法镜头在最终留出的 `+0.064R` 高于精简团队，但最终留出没有选择权，不能用这个事后结果推翻验证阶段已经冻结的团队。只有 `BTCUSDT|1h` 与 `ETHUSDT|4h` 同时满足验证、最终留出均为正且各自不少于 20 笔，因此部署模式固定为：
...
- `tools/requirements-plan-gate.txt`
...
`tools/update_data.sh full` 在检测到本地档案目录时也会调用这条完整训练链。模型产物记录 Python、NumPy、Pandas 与 scikit-learn 版本；当前固定运行时见 `training_runtime`。`.gitignore` 排除 `enent contract/` 和临时训练目录；原始订单、执行 ID、trade ID 与分钟 K 线不会进入 GitHub。
