...
| 长历史教练训练 | 已完成 | Paul 真实账本 + BTC/ETH/NQ/ES；12 个范围，开发/验证/最终留出严格按时间切分 |
...
17 个规则化方法镜头均被单独拿出来做最终留出压力测试。所有可交易镜头的范围加权 EV 都为负；最接近中性的动量镜头仍为 `-0.088R`。网格、情绪、宏观因为本版没有独立方向或完整历史输入而保持待校准。
